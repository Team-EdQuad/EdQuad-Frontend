  ...
  return (
    <Box sx={{ textAlign: "center", userSelect: "none" }}>
      <svg height={radius * 2} width={radius * 2}>
        <defs>
          <linearGradient id={`gradient-${label}`} x1="1" y1="0" x2="0" y2="1">
...
const AttendanceOverview = ({ studentId, classId }) => {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        ...
        const weeklyData = await getWeeklyAttendanceRate(studentId, classId);
        const attendanceRatedata = await getAttendanceRate(studentId, classId);
        const engagementScoreData = await getEngagementScore(studentId, classId);

        const dataMap = new Map(
          data.map((item) => [item.month, item.attendance_rate])
        );
        const weeklyDataMap = new Map(
          weeklyData.map((item) => [item.weekday, item.attendance_rate])
        );

        const attendanceRates = months.map((month) => dataMap.get(month) || 0);
        const weeklyAttendanceRates = weekdays.map(
          (weekday) => weeklyDataMap.get(weekday) || 0
        );

        const nonAcademicResponse = await getNonacademicAttendance(studentId, classId);
        const labels = nonAcademicResponse.activities_attendance.map(
          (item) => item.name
        );
        const presentData = nonAcademicResponse.activities_attendance.map(
          (item) => item.present
        );
        const absentData = nonAcademicResponse.activities_attendance.map(
          (item) => item.absent
        );
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <Button
              ...
              onClick={() =>
                (window.location.href = "/student/attendance/analysis")
              }
            >
              View Attendance History
            </Button>
      ...
      <Grid container spacing={6} sx={{ mt: 5, ml: 5, mr: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mt: 3, ml: 5, mr: 3 }}>
        <Box
          display={"flex"}
          flexDirection="column"
          alignItems="left"
